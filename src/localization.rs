...
pub fn help_message(user: &Option<User>) -> &'static str {
    match get_language(user) {
        ...
        _ => "*Hello!* I'm a bot that can extract images from .attheme files or put images in them. To extract a wallpaper, just send me a theme. If you want to put a wallpaper, send me the theme and then send the image (as an image or a `jpg`, `png`, `bmp`, `tiff`, `webp` document) in reply to that theme. I'll reply you with the new theme removing the old wallpaper.",
    }
}
...
pub fn image_with_no_reply(user: &Option<User>) -> &'static str {
    match get_language(user) {
        ...
        _ => "Ehm, if you want me to put a wallpaper inside a theme, you should reply to the message with that theme.",
    }
}
...
pub fn theme_caption(user: &Option<User>) -> &'static str {
    match get_language(user) {
        ...
        _ => "Nice wallpaper, I've added it to the theme!",
    }
}
...
pub fn theme_with_no_image(user: &Option<User>) -> &'static str {
    match get_language(user) {
        ...
        _ => "Hmm, it looks like your theme doesn't have a wallpaper. But I can put one inside it! Just send an image in reply to this theme.",
    }
}
